...
export class FeedDAOImpl implements FeedDAO{
    ...
    async addFeed(author_alias: UserDto, timeStamp: number, status: string, receiver: string) {
        
        try {
        const params = {
            TableName: this.tableName,
            Item: {
                [this.receiver_alias_attr]: receiver,
            ...
            console.log(`Status by ${author_alias.alias} added to feeds of ${receiver} users.`);
        } catch (error) {
    ...
    async addFeedsBatch(author_alias: UserDto, timeStamp: number, status: string, receivers: string[]): Promise<void> {
